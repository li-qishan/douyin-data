...
function draw(data) {
  ...
  //$("#segmented_c1_inside").attr("fill","red")
  ...
  //var background_img = config.background_img;

  d3.select("body").attr("style", "background:" + background_color);
  //d3.select("body").attr("style", "background-image:" + background_img);
  ...
  if (showMessage) {
    ...
    // 右1文字
    g.insert("text")
      .attr("class", "growth_source")
      //数值 年月日
      .attr("x", 700)
      //适应 底部margin 50 .attr("y", 2100)
      .attr("y", 2100)
      //.attr("y", 1800)
      //适应 底部margin 500 ---1800
      // .attr("x", 750)
      // .attr("y", 2050)
      // .attr("x", text_x)
      // .attr("y", text_y)
  ...
  function redraw() {
    ...
    if (big_value) {
      ...
      xScale
      //增加 貌似是间距
        .domain([0, d3.max(currentData, xValue) + 2000])
        .range([0, innerWidth]);
    }
    if (auto_sort) {
      dateLabel
